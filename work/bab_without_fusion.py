...
def read_data():
    ...
    # 确保 'text' 列中的所有值为字符串类型，并替换空值为 ''
    texts = df['text'].fillna('').astype(str).tolist()
    labels = df['label'].tolist()  # label
    return texts, labels
...
class ScoringModel(nn.Module):
    def __init__(self, base_model_path, num_classes):
        ...
        self.classifier = nn.Linear(self.config.hidden_size, num_classes)  # 输出维度为 num_classes
...
def test(model, test_dataloader, epoch, device, results):
    ...
    p = precision_score(true_labels, predictions, average='weighted', zero_division=0)
    r = recall_score(true_labels, predictions, average='weighted', zero_division=0)
...
# 训练函数
def train(batch_size, num_epochs, num_classes):
    ...
    model = ScoringModel(base_pre_model_path, num_classes=num_classes)  # 传入类别数
    loss_fn = torch.nn.CrossEntropyLoss()
    ...
    df_results.to_excel('./THUNews.xlsx', index=False, sheet_name='Results')

    model_save_path = './trained_bab.pt'  # 模型保存路径
    torch.save(model.state_dict(), model_save_path)


if __name__ == "__main__":
    base_pre_model_path = './bert-base-uncased'
    batch_size = 2
    num_epochs = 50
    num_classes = 6  # 类别数
    ...
    train(batch_size, num_epochs, num_classes)