...
def encode_texts(texts):
    ...
    for text_group in texts:  # text_group是包含7个文本的列表
        ...
        for text in text_group:
            if text:
                encoded = tokenizer(text, padding='max_length', max_length=256, truncation=True, return_tensors="pt")
                ...
                encoded_group['input_ids'].append(torch.zeros(256, dtype=torch.long))
                encoded_group['attention_mask'].append(torch.zeros(256, dtype=torch.long))
        # 将编码后的文本段合并为批次维度
...
class ScoringModel(nn.Module):
    def __init__(self, base_model_path):
        ...
        # 双向LSTM层
        self.bilstm = nn.LSTM(input_size=self.config.hidden_size, hidden_size=self.config.hidden_size // 2, num_layers=1, batch_first=True, bidirectional=True)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        cls_representation = outputs.last_hidden_state[:, 0, :].view(batch_size, num_segments, -1)  # [batch_size, 7, hidden_size]

        # 去掉多头注意力层，直接将cls_representation输入到BiLSTM
        lstm_output, (h_n, c_n) = self.bilstm(cls_representation)
...
def train(batch_size, num_epochs):
    ...
    # 将结果保存到一个Excel工作表中
    df_results.to_excel('./results/xiaoya_without_att.xlsx', index=False, sheet_name='Results')
